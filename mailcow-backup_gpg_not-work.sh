...
ENCRYPTED_ARCHIVE_NAME="$ARCHIVE_NAME.gpg"      # Ім'я зашифрованого архіву
...
GPG_RECIPIENT="543543654654"         				# GPG ключ для шифрування
...
if ! command -v gpg &> /dev/null; then
    log "Помилка: gpg не встановлений!"
    exit 1
fi
...
# Шифрування архіву GPG
log "Шифрування архіву..."
gpg --encrypt --recipient "$GPG_RECIPIENT" --output "$BACKUP_DIR/$ENCRYPTED_ARCHIVE_NAME" "$BACKUP_DIR/$ARCHIVE_NAME"

# Перевірка, чи зашифрований архів створився
if [[ ! -f "$BACKUP_DIR/$ENCRYPTED_ARCHIVE_NAME" ]]; then
    log "Помилка: Не вдалося зашифрувати архів!"
    exit 1
fi

# Видалення незашифрованого архіву
rm -f "$BACKUP_DIR/$ARCHIVE_NAME"

# Відправка зашифрованого архіву у S3
log "Відправка зашифрованого архіву в S3..."
rclone copy "$BACKUP_DIR/$ENCRYPTED_ARCHIVE_NAME" "$S3_REMOTE"

# Перевірка, чи архів успішно збережений у S3
if rclone ls "$S3_REMOTE" | grep -q "$ENCRYPTED_ARCHIVE_NAME"; then
    log "Архів успішно збережений у S3, видалення локальних файлів..."
    rm -rf "$BACKUP_DIR/$ENCRYPTED_ARCHIVE_NAME"
else
...
find "$BACKUP_DIR" -type f -name "mailcow-*.tar.gz.gpg" -mtime +7 -delete

# Записуємо лог про успішне завершення резервного копіювання
log "Бекап $ENCRYPTED_ARCHIVE_NAME успішно створено, зашифровано, завантажено в S3 та видалено локально."
